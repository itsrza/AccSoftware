...
test('version:consistent',()=>{const v=pkg().version;assert(v&&v===tauriConfig().version,'UI/Tauri version mismatch');assert(read(files.cargo).includes('version.workspace = true'),'host must use workspace version');assert(new RegExp(`"version": *"${v}"`).test(read(path.join(root,'package.json'))),'root version mismatch')})
...
/* «کامیت‌نشده» معیار است نه «وجود روی دیسک» — روی ماشین توسعه node_modules هست. */
const tracked=p=>{try{return execSync(`git ls-files --error-unmatch ${p}`,{stdio:'pipe'}).toString().trim().length>0}catch{return false}}
test('artifacts:no-node_modules',()=>assert(!tracked('apps/desktop-ui/node_modules'),'node_modules must not be committed'))
test('artifacts:no-dist',()=>assert(!tracked('apps/desktop-ui/dist'),'dist must not be committed'))
...
/* کدِ ارسالی (bundle) ملاک است، نه ابزارِ تستِ خودِ audit — __tests__ حذف می‌شود. */
const runtimeTree=()=>readTree(path.join(src,'components'))+'\n'+readTree(path.join(src,'pages'))+'\n'+readTree(path.join(src,'lib','i18n'))+'\n'+read(src+'/App.tsx')+'\n'+read(src+'/api.ts')+'\n'+read(src+'/main.tsx')
test('frontend:no-child-process',()=>assert(!runtimeTree().match(/child_process|execFile|spawn\s*\(/),'native process execution must stay out of frontend'))
...
/* زبانِ فعال در localStorage نگهداری می‌شود — غیرحساس و پیش‌فرضِ فارسی؛
   بقیه‌ی source نباید هیچ حالت حساسی در storage مرورگر بگذارد. */
const storageSafe=()=>readTree(path.join(src,'lib','i18n'))
test('frontend:no-local-storage',()=>assert(!readTree(src).replace(storageSafe(),'').match(/localStorage\s*\./),'sensitive app state must not use localStorage'))
...
test('rust:sqlite-bundled',()=>assert(read(files.cargo).includes('rusqlite')&&(read(files.cargo).includes('bundled')||read(path.join(root,'Cargo.toml')).includes('bundled')),'SQLite must be bundled'))
...
test('rust:serde',()=>assert(read(files.cargo).includes('serde')||read(path.join(root,'Cargo.toml')).includes('serde'),'serde dependency missing'))
test('rust:tauri-v2',()=>assert(/tauri(\.workspace)? *= *\{?[^\n]*"2"|tauri = "2"/.test(read(files.cargo)),'Tauri 2 dependency missing'))
