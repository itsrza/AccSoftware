...
fn get_account_ledger(
    ...
    // گزارش فقط-خواندنی است؛ تراکنش صرفاً برای active_context باز شد و
    // باید همان‌جا بسته شود تا قفل نوشتنِ اتصال تا سقوط تابع نماند.
    drop(st);
    tx.commit().map_err(|e| e.to_string())?;
    Ok(out)
}
...
fn get_party_balances(state: &State<AppState>, sales: bool) -> Result<Vec<PartyBalance>, String> {
    ...
    // مقدار ثابت از بولین — با گارد صریح، مسیر تزریق بسته می‌ماند
    let inv_type = if sales { "sales" } else { "purchase" };
    debug_assert!(matches!(inv_type, "sales" | "purchase"));
...
fn register_plugin(
    ...
    for p in perms {
        ...
        let permission = p
            .as_str()
            .ok_or_else(|| "PLUGIN-027: فهرست مجوزهای افزونه باید متنی باشد".to_string())?;
        tx.execute(
...
fn main() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            api_profiles::list_api_profiles,
            api_profiles::create_api_profile,
