...
fn set_inventory_valuation_method(state: State<AppState>, method: String) -> Result<(), String> {
    if !["fifo", "moving_average", "weighted_average"].contains(&method.as_str()) {
        return Err("INV-101: روش ارزش‌گذاری نامعتبر است".into());
    }
    let mut c = conn(&state)?;
...
fn set_inventory_count_line(
    ...
    tx.execute("UPDATE inventory_count_lines SET counted_quantity=?,recount_quantity=?,variance=?-system_quantity,note=?,status=CASE WHEN ? IS NULL THEN 'counted' ELSE 'recounted' END WHERE id=?",params![counted_quantity,recount_quantity,counted_quantity,note,recount_quantity,line_id])?;
    tx.execute(
...
fn post_inventory_count(state: State<AppState>, session_id: String) -> Result<(), String> {
    ...
    let incomplete:i64=tx.query_row("SELECT COUNT(*) FROM inventory_count_lines WHERE session_id=?1 AND counted_quantity IS NULL",params![session_id],|r|r.get(0))?;
    if incomplete > 0 {
        return Err("INV-136: همه اقلام باید شمارش شوند".into());
    }
    let wh: String = tx.query_row(
    ...
    for (pid, _sys, counted, var, lid) in items {
        if var.abs() > 0.0000001 {
            tx.execute("INSERT INTO inventory_balances(product_id,warehouse_id,quantity) VALUES(?,?,?) ON CONFLICT(product_id,warehouse_id) DO UPDATE SET quantity=excluded.quantity,updated_at=CURRENT_TIMESTAMP",params![pid,wh,counted])?;
            tx.execute("INSERT INTO inventory_movements(id,company_id,product_id,warehouse_id,movement_type,quantity,unit_cost,reference_type,reference_id,note,created_by) VALUES(?,?,?,?,?,?,?,?,?,?,?)",params![format!("count-move-{}",lid),company,pid,wh,"adjustment",var.abs(),0,"inventory_count",session_id,format!("variance:{}",var),user])?;
        }
    }
    tx.execute("UPDATE inventory_count_sessions SET status='posted',posted_at=CURRENT_TIMESTAMP WHERE id=?",params![session_id])?;
    audit(
        &tx,
        &user,
...
fn create_inventory_transfer_order(
    ...
    tx.execute("INSERT INTO inventory_transfer_orders(id,company_id,product_id,from_warehouse_id,to_warehouse_id,quantity,unit_cost,status,note,created_by) VALUES(?,?,?,?,?,?,?,'in_transit',?,?)",params![id,company,product_id,from_warehouse_id,to_warehouse_id,quantity,unit_cost,note,user])?;
    tx.execute("UPDATE inventory_balances SET quantity=quantity-?,in_transit_quantity=in_transit_quantity+?,updated_at=CURRENT_TIMESTAMP WHERE product_id=? AND warehouse_id=?",params![quantity,quantity,product_id,from_warehouse_id])?;
    audit(
        &tx,
        &user,
...
fn receive_inventory_transfer(state: State<AppState>, transfer_id: String) -> Result<(), String> {
    ...
    tx.execute("UPDATE inventory_balances SET in_transit_quantity=MAX(0,in_transit_quantity-?),updated_at=CURRENT_TIMESTAMP WHERE product_id=? AND warehouse_id=?",params![row.5,row.1,row.2])?;
    tx.execute("INSERT INTO inventory_balances(product_id,warehouse_id,quantity) VALUES(?,?,?) ON CONFLICT(product_id,warehouse_id) DO UPDATE SET quantity=quantity+excluded.quantity,updated_at=CURRENT_TIMESTAMP",params![row.1,row.3,row.5])?;
    tx.execute("INSERT INTO inventory_movements(id,company_id,product_id,warehouse_id,movement_type,quantity,unit_cost,reference_type,reference_id,created_by) VALUES(?,?,?,?,?,?,?,?,?,?)",params![format!("transfer-out-{}",transfer_id),row.0,row.1,row.2,"transfer_out",row.5,row.6,"transfer",transfer_id,user])?;
    tx.execute("INSERT INTO inventory_movements(id,company_id,product_id,warehouse_id,movement_type,quantity,unit_cost,reference_type,reference_id,created_by) VALUES(?,?,?,?,?,?,?,?,?,?)",params![format!("transfer-in-{}",transfer_id),row.0,row.1,row.3,"transfer_in",row.5,row.6,"transfer",transfer_id,user])?;
    tx.execute("UPDATE inventory_transfer_orders SET status='received',received_at=CURRENT_TIMESTAMP WHERE id=?",params![transfer_id])?;
    audit(
        &tx,
        &user,
...
fn update_check_status(
    ...
    if new_status == "cleared" {
        ...
        let treasury_account = treasury_account
            .flatten()
            .ok_or("CHK-013: حساب خزانه به حسابداری متصل نیست".to_string())?;
...
fn post_return(state: &State<AppState>, return_id: String, sale: bool) -> Result<(), String> {
    ...
    let wid = row.3.ok_or("RET-008: انبار برگشت مشخص نیست".to_string())?;
    let mut st = tx
        .prepare(&format!(
