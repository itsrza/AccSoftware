...
export function Invoices({page}:{page:string,onNavigate?:(p:string)=>void}){
 ...
 useEffect(()=>{let active=true;setLoading(true);setError('');(sale?getSalesInvoices():getPurchaseInvoices()).then(v=>{if(active)setRows(v)}).catch(e=>{if(active)setError(String(e))}).finally(()=>{if(active)setLoading(false)});return()=>{active=false}},[sale])
 ...
 return <section className="page" dir="rtl">
  ...
  <div className="panel list-panel"><div className="toolbar"><div className="global-search inner"><Icon name="search" size={17}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="جستجو در فاکتورها..."/></div><button className="filter-btn" disabled><Icon name="filter"/> فیلترها</button><button className="filter-btn" disabled><Icon name="download"/> خروجی</button><button className="icon-btn" onClick={()=>{setLoading(true);(sale?getSalesInvoices():getPurchaseInvoices()).then(setRows).catch(e=>setError(String(e))).finally(()=>setLoading(false))}}><Icon name="refresh"/></button></div>
